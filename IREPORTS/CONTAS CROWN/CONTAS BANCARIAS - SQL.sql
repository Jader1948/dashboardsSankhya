...
FROM (SELECT
  ...
  AND T7.DTLANC >= cast(cast(year(getdate()) as char(4))+'-'+cast(month(getdate()) as char(2))+'-01' as date)
  AND T7.DTLANC < cast(cast(year(getdate()) as char(4))+'-'+cast(month(getdate()) as char(2))+'-'+cast(day(getdate()) as char(2)) as date)), 0)
  AS SALDO,
  CTA.CODEMP,
  CTA.CODPARC,
  BCO.NOMEBCO
...
FROM TGFMBC T3
WHERE T3.CODCTABCOINT =  5  AND T3.CONCILIADO =  'S' AND T3.DHCONCILIACAO  >= '2022/01/01' AND T3.DHCONCILIACAO  < GETDATE()
...
SELECT 422 AS CODBC,
 ...
 CAST(max(t3.DHCONCILIACAO) as date) AS ULT_CONCILIACAO 
FROM TGFMBC T3 
WHERE T3.CODCTABCOINT =  6  AND T3.CONCILIADO =  'S' AND T3.DHCONCILIACAO  >= '2022/01/01' AND T3.DHCONCILIACAO  < getdate()
